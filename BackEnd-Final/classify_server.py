...
def classify():
    ...
    print type(name)
    listTmp = mecab.pos(name)
    finder = collocations.BigramCollocationFinder.from_words(listTmp)
    listTmp = finder.nbest(bigram_measures.pmi, 3)
    
    for node in listTmp:
        name += " " + node[0][0]+node[1][0] +" "+ node[1][0] + node[0][0] + " " + node[0][0] + " " + node[1][0]
